...
int main(void)
{
    ...
    GLFWwindow *window = glfwCreateWindow(g_windowWidth, g_windowHeight, "Object Viewer", NULL, NULL);
    if (!window)
    {
    ...
    if (glfwGetWindowAttrib(window, GLFW_VISIBLE)){ //first time buffer appears
    ...
    //заполнение информацией (позициями вертекса на видеокарту ???),для текущего binded буфера
